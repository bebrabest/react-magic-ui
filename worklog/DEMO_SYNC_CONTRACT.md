...
- `switch-missing-accessible-name` - if the demo renders `<Switch />`, it should provide a real accessible name via `label`, `aria-label`, or `aria-labelledby`; nearby visual text alone is not a reliable control label contract
