...
- `input-missing-accessible-name` / `select-missing-accessible-name` / `checkbox-missing-accessible-name` / `switch-missing-accessible-name` / `slider-missing-accessible-name` / `button-missing-accessible-name` / `badge-missing-content` - if the demo renders `<Input />`, `<Select />`, `<Checkbox />`, `<Switch />`, `<Slider />`, `<Button />`, or `<Badge />`, it should expose a real user-facing label/content; for Input/Select/Checkbox/Switch that means `label`, `aria-label`, or `aria-labelledby` (not placeholder text alone), for Slider that means `aria-label` or `aria-labelledby`, for Button that means visible text children, the `text` prop, or `aria-label` / `aria-labelledby` for icon-only actions, and for Badge that means visible text children or at minimum `aria-label` / `aria-labelledby` if it is intentionally icon-only
