...
- `input-missing-accessible-name` / `select-missing-accessible-name` / `checkbox-missing-accessible-name` / `switch-missing-accessible-name` / `slider-missing-accessible-name` / `button-missing-accessible-name` / `badge-missing-content` / `card-missing-content` / `glass-missing-content` / `topbar-missing-content` - if the demo renders `<Input />`, `<Select />`, `<Checkbox />`, `<Switch />`, `<Slider />`, `<Button />`, `<Badge />`, `<Card>`, `<Glass>`, or `<Topbar>`, it should expose a real user-facing label/content; for Input/Select/Checkbox/Switch that means `label`, `aria-label`, or `aria-labelledby` (not placeholder text alone), for Slider that means `aria-label` or `aria-labelledby`, for Button that means visible text children, the `text` prop, or `aria-label` / `aria-labelledby` for icon-only actions, for Badge that means visible text children or at minimum `aria-label` / `aria-labelledby` if it is intentionally icon-only, for Card and Glass that means visible child content or at minimum `aria-label` / `aria-labelledby` if the wrapper is intentionally decorative, and for Topbar that means visible brand/title content (for example an `h1` or `Topbar.Brand title`) or an explicit accessible name for intentionally minimal headers
...
- `sidebar-duplicate-item-ids` / `sidebar-item-target-missing` / `sidebar-active-item-mismatch` / `sidebar-item-click-target-mismatch` - sidebar navigation should keep each `<Sidebar.Item itemId>` unique, aligned with a real demo section/container `id`, consistent with the initial `activeItemId` state, and consistent with any inline `handleSidebarChange(...)` click target so active-state tracking and scroll navigation do not quietly drift apart
- `sidebar-item-missing-accessible-name` - every demo `<Sidebar.Item>` should expose a usable accessible name (`aria-label` or plain text children), otherwise the navigation can quietly degrade into unnamed items for assistive tech even before collapse enters the picture
