...
- `sidebar-duplicate-item-ids` / `sidebar-item-target-missing` / `sidebar-active-item-mismatch` - sidebar navigation should keep each `<Sidebar.Item itemId>` unique, aligned with a real demo section/container `id`, and consistent with the initial `activeItemId` state so active-state tracking does not quietly boot into an ambiguous or dead selection
