...
- `input-missing-accessible-name` / `checkbox-missing-accessible-name` / `switch-missing-accessible-name` / `slider-missing-accessible-name` / `button-missing-accessible-name` - if the demo renders `<Input />`, `<Checkbox />`, `<Switch />`, or `<Button />`, it should provide a real accessible name; for Input/Checkbox/Switch that means `label`, `aria-label`, or `aria-labelledby` (not placeholder text alone), for Slider that means `aria-label` or `aria-labelledby`, and for Button that means visible text children, the `text` prop, or `aria-label` / `aria-labelledby` for icon-only actions
