...
- `input-missing-accessible-name` / `select-missing-accessible-name` / `checkbox-missing-accessible-name` / `switch-missing-accessible-name` / `slider-missing-accessible-name` / `button-missing-accessible-name` / `badge-missing-content` / `topbar-missing-content` - if the demo renders `<Input />`, `<Select />`, `<Checkbox />`, `<Switch />`, `<Slider />`, `<Button />`, `<Badge />`, or `<Topbar>`, it should expose a real user-facing label/content; for Input/Select/Checkbox/Switch that means `label`, `aria-label`, or `aria-labelledby` (not placeholder text alone), for Slider that means `aria-label` or `aria-labelledby`, for Button that means visible text children, the `text` prop, or `aria-label` / `aria-labelledby` for icon-only actions, for Badge that means visible text children or at minimum `aria-label` / `aria-labelledby` if it is intentionally icon-only, and for Topbar that means visible brand/title content (for example an `h1` or `Topbar.Brand title`) or an explicit accessible name for intentionally minimal headers
