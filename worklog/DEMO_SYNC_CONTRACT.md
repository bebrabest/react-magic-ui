...
- `checkbox-missing-accessible-name` / `switch-missing-accessible-name` / `button-missing-accessible-name` - if the demo renders `<Checkbox />`, `<Switch />`, or `<Button />`, it should provide a real accessible name; for Checkbox/Switch that means `label`, `aria-label`, or `aria-labelledby`, and for Button that means visible text children, the `text` prop, or `aria-label` / `aria-labelledby` for icon-only actions
